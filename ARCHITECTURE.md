...
| Video | External embeds only (YouTube, YouTube Shorts, Vimeo, Loom, Google Drive, Instagram posts/Reels/reels/TV, direct MP4/WebM/OGG + Cloudinary) | Zero storage cost; parsed by a declarative `parseVideoUrl` registry (`src/lib/video`) and lazy-loaded iframes for performance. Single source of truth shared by `normalizeVideoUrl` (`src/lib/embed`) used in API persistence and `getEmbedUrl` used by renderers; each platform exposes an `aspectRatio` so players size correctly (9:16 / 1:1 / 16:9). |
